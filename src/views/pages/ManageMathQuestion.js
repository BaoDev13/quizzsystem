...
import CreateQuestionM from "./MathQuiz/CreateQuestionM";
import EditQuestionM from "./MathQuiz/EditQuestionM";

const ManageMathQuiz = () => {
  ...
  const fetchQuestions = async () => {
    try {
      const questionsRef = collection(db, "quizpapers/ppr003/questions");
      ...
      for (const questionDoc of questionsSnapshot.docs) {
        ...
        const answersRef = collection(
          db,
          `quizpapers/ppr003/questions/${questionId}/answers`
        );
  ...
  const fetchQuizSettings = async () => {
    try {
      const quizSettingsDocRef = doc(db, "quizpapers/ppr003");
  ...
  const handleDeleteQuestion = async (questionId) => {
    try {
      ...
      const questionDocRef = doc(
        db,
        `quizpapers/ppr003/questions/${questionId}`
      );
      batch.delete(questionDocRef);
      await batch.commit();
      const countDocRef = doc(db, `quizpapers/ppr003`);
      ...
      <Container className="mt--7" fluid>
        <Card className="bg-secondary shadow">
          <CardHeader className="bg-transparent border-0">
            <h2 className="mb-5">Manage Math Quiz</h2>
            <Button
              color="primary"
              onClick={() => {
                setQuizType("Math Mania");
                toggleModal();
              }}
            >
